...
let type_of_const color c t setC =
...
let type_of_var color tenv x t setC =
...
let rec constraintGen color tenv e t setC (*(ETnode expType)*) =
	(print_concrete_expr e); print_string(" : "); (print_type t); print_newline();
	(*expType.etLink <- ETnode (expTypePair e t);*)
	match e with
	| Var x -> type_of_var color tenv x t setC
	| Const c -> type_of_const color c t setC
	| Fun (x, e) -> (* T-ABS *)
		let tX = tevar() and tbody = tevar() in
		let setC1 = constraintGen color (extend tenv (x, tX)) e tbody setC  in
		(* fixme *)
	...
	| App (e1, e2) -> (* T-APP *)
		...
		let setC1 = constraintGen color tenv e1 te1 setC  in
		let setC2 = constraintGen color tenv e2 te2 setC  in
	...
	| Let (x, e1, e2) -> (* T-LET *) (* fixme: In paper, add the case for [let a b c ... = e1 in e2] *) (* also, implement the generalization *)
		...
		let setC1 = constraintGen color tenv func te1 setC in
		let setC2 = constraintGen color (extend tenv (List.hd x, tfunc)) e2 te2 setC in
		link tfunc te1; link t te2; addC tfunc setC; addC t setC
	| LetP (x, e1, e2) ->
		let tfunc = tevar() and te1 = tevar() and te2 = tevar() in
		let func = create_lam_expr (List.tl x) e1 in
		constraintGen "policy" tenv func te1 setC;
		constraintGen color (extend tenv (List.hd x, tfunc)) e2 te2 setC;
		link tfunc te1; link t te2; addC tfunc setC; addC t setC

let genConstrSet e t cs (*etPair*) = constraintGen "code" [] e t cs (*etPair*);;
